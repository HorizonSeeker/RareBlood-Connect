...
export async function POST(req) {
  try {
    ...
    if (token && isLoggedIn) {
      ...
      // If this is a hospital user, ensure they are verified before allowing this operation
      if (user.role === 'hospital') {
        const hv = await ensureHospitalVerified(user._id);
        if (!hv.success) {
          return NextResponse.json({ error: hv.error || 'Hospital account not verified', verification_status: hv.verification_status }, { status: hv.status });
        }
      }
    }
    ...
    (async () => {
      try {
        ...
        await pusher.trigger('blood-channel', 'new-alert', {
          message: `🆘 EMERGENCY: Blood type ${emergencyRequest.blood_type} (${emergencyRequest.units_required} unit) urgently needed - PATIENT: ${patientName}`,
          requestId: emergencyRequest._id,
          bloodbankId: emergencyRequest.bloodbank_id,
          isEmergency: true
        });
        console.log("--> Emergency notification sent!");
      } catch (pusherErr) {
        console.error('❌ Pusher trigger error (non-blocking):', pusherErr);
        // Don't fail the main request if Pusher fails
      }
    })().catch(err => console.error('Async Pusher task error:', err));
