...
export async function POST(request) {
  try {
    ...
    const { donorId, bloodType, urgencyLevel, message, requestType = 'CONTACT' } = await request.json();
    
    // Validate request type
    if (!['CONTACT', 'EMERGENCY'].includes(requestType)) {
      return NextResponse.json(
        { error: 'Invalid request type. Must be CONTACT or EMERGENCY' },
        { status: 400 }
      );
    }
    ...
    const contactRequest = new DonorContactRequest({
      ...
      sourceType: requester.role === 'bloodbank_admin' ? 'blood_bank' : 'donor_search'
    });
    ...
    if (fcmToken) {
      try {
        const fcmMessage = {
          notification: {
            title: '🩸 Blood Request from ' + requester.name,
            body: `Urgent request for ${bloodType} blood (${urgencyLevel || 'Medium'} priority)`
          },
          data: {
        ...
        console.log('✅ FCM notification sent to donor:', fcmResponse);
      } catch (fcmError) {
