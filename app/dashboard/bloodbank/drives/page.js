...
export default function BloodbankDrives() {
  ...
  const validateForm = () => {
    ...
    if (formData.date && formData.date < getTodayDate()) {
      ...
      <ProtectedRoute allowedRoles={['bloodbank_admin']}>
        <div className="min-h-screen bg-[var(--background)] flex items-center justify-center">
          <div className="text-center">
            <div className="rounded-full h-16 w-16 border-b-2 border-[#ef4444] mx-auto"></div>
            ...
            <div className="bg-[var(--card-background)] rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-[var(--text-primary)] mb-2">
                      Description * <span className="text-xs text-[var(--text-secondary)]">(Max 500 characters)</span>
                    </label>
                    <textarea
                      required
                      rows={3}
                      ...
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:border-transparent bg-[var(--background)] text-[var(--text-primary)] ${
                        formErrors.description ? 'border-red-500 focus:ring-red-500' : 'border-[var(--border)] focus:ring-[#ef4444]'
                      }`}
                      placeholder="Describe the donation drive"
                    />
                    <div className="flex justify-between items-center mt-2">
                      <span className={`text-xs ${
                        formData.description.length > 450 ? 'text-orange-600' : 'text-[var(--text-secondary)]'
                      }`}>
                        {formData.description.length}/500 characters
                      </span>
                      {formErrors.description && (
                        <span className="text-xs text-red-600 font-medium">{formErrors.description}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:border-transparent bg-[var(--background)] text-[var(--text-primary)] ${
                        formErrors.date ? 'border-red-500 focus:ring-red-500' : 'border-[var(--border)] focus:ring-[#ef4444]'
                      }`}
                    />
                    {formErrors.date && (
                      <span className="block text-xs text-red-600 font-medium mt-1">{formErrors.date}</span>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <TimeSelect
                      ...
                      onChange={(value) => {
                        ...
                        // Validate immediately and clear error if valid
                    ...
                    />
                    {formErrors.end_time && (
                      <span className="block text-xs text-red-600 font-medium mt-1">{formErrors.end_time}</span>
                    )}
                  </div>
                </div>
