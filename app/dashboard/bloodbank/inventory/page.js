...
const InventoryManagement = () => {
  ...
  const totalUnits = displayedInventory.reduce((sum, item) => sum + item.units_available, 0);
    ...
    <div className="min-h-screen bg-[var(--background)] py-8">
      <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {bloodTypes
              .filter(bloodType => showDepleted || (groupedInventory[bloodType]?.total || 0) > 0)
              .map(bloodType => (
                ...
                {groupedInventory[bloodType]?.items?.length > 0 ? (
                  <div className="space-y-2">
                    {groupedInventory[bloodType].items.map(item => {
                      const isDepleted = item.units_available === 0;
                      return (
                        <div 
                          key={item._id} 
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span className={`text-sm font-medium ${isDepleted ? 'text-gray-500' : ''}`}>
                              {item.units_available} units
                              {isDepleted && <span className="ml-2 text-xs bg-red-100 text-red-700 px-2 py-1 rounded">Out of Stock</span>}
                            </span>
                            <div className="flex items-center space-x-1">
                              <div className="group relative">
                                <button
                                  onClick={() => !isDepleted && handleEdit(item)}
                                  disabled={isDepleted}
                                  ...
                                  title={isDepleted ? 'Out of stock - cannot edit' : 'Edit'}
                                >
                              ...
                              <div className="group relative">
                                <button
                                  onClick={() => !isDepleted && handleDelete(item._id)}
                                  disabled={isDepleted}
                                  className={`p-1 rounded ${
                                    isDepleted
                                      ? 'text-gray-400 cursor-not-allowed'
                                  ...
                                  title={isDepleted ? 'Out of stock - cannot delete' : 'Delete'}
                                >
                                  <Trash2 className="h-4 w-4" />
                                </button>
                                {isDepleted && (
                                  <div className="absolute bottom-full right-0 mb-2 px-2 py-1 bg-gray-900 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                    Out of stock
                                  </div>
                                )}
                              </div>
