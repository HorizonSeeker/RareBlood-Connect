...
const HospitalInventoryManagement = () => {
  ...
  // Quick deduct modal state
  const [showQuickDeductModal, setShowQuickDeductModal] = useState(false);
  const [quickDeductItem, setQuickDeductItem] = useState(null);
  const [quickDeductUnits, setQuickDeductUnits] = useState('');
  const [deductLoading, setDeductLoading] = useState(false);
  
  // Toast warning tracking (prevent spam)
  const lastAlertTimeRef = useRef(0);
  const ALERT_COOLDOWN_MS = 10000; // 10 seconds

  const bloodTypes = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'];
  const displayedInventory = showDepleted 
    ? inventory 
    : inventory.filter(item => item.units_available > 0);
  const actions = [
      ...
      try {
        ...
        if (previousStatusRef.current && previousStatusRef.current !== newStatus) {
          if (newStatus === 'verified') {
            ...
            error('❌ Xin lỗi, đơn xác minh bệnh viện của bạn bị từ chối');
          }
        }
  ...
  const fetchInventory = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Show alerts with cooldown to prevent spam
        const now = Date.now();
        if (now - lastAlertTimeRef.current >= ALERT_COOLDOWN_MS && data.summary) {
          if (data.summary?.lowStockAlerts?.length > 0) {
            error(`⚠️ Low stock alert: ${data.summary.lowStockAlerts.length} blood type(s) below minimum level`);
          }
  ...
  const handleAction = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Fetch both inventory and logs data simultaneously
        await Promise.all([
          fetchInventory(),
          fetchLogs()
        ]);
        ...
        <div className="container mx-auto px-4">
          ...
          {activeTab === 'inventory' && (
            <>
              ...
              {/* Show Depleted Stock Toggle */}
              <div className="mb-6 flex items-center space-x-3 p-4 bg-[var(--card-background)] rounded-lg border border-[var(--border-color)]">
                <label className="flex items-center space-x-2 cursor-pointer">
                  ...
                  />
                  <span className="text-[var(--text-primary)] font-medium">Show Depleted Stock</span>
                </label>
                <span className="text-sm text-[var(--text-secondary)]">
                  ({displayedInventory.length} items displayed)
                </span>
              </div>
                    ...
                    <tbody className="bg-[var(--card-background)] divide-y divide-[var(--border-color)]">
                      {displayedInventory.length === 0 ? (
                        ...
                        displayedInventory.map((item) => {
                          ...
                          return (
                            <tr key={item._id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 ${isExpired ? 'bg-red-50/30 dark:bg-red-900/10' : ''}`}>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div className="flex items-center justify-center space-x-2">
                                  <button
                                    ...
                                    title="Manage Units (Reserve, Mark Expired)"
                                  >
              ...
              <div className="overflow-x-auto">
                {logsLoading ? (
                  <div className="p-8 text-center">
                    <RefreshCw className="w-6 h-6 text-[#ef4444] mx-auto mb-2" />
