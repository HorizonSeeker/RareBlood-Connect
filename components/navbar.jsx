...
const Navbar = () => {
  ...
  const getDashboardLink = () => {
    // Single entry point for all roles; dispatcher will redirect appropriately
  ...
  const getRoleSpecificNavItems = () => {
    ...
    if (isDonor) {
      items.push(
        ...
        { href: '/dashboard/donor', icon: Activity, label: 'Dashboard', active: pathname === '/dashboard/donor' }
      );
    } else if (isBloodBank) {
      items.push(
        { href: '/dashboard/bloodbank/appointments', icon: Calendar, label: 'Donation Appointments', active: pathname === '/dashboard/bloodbank/appointments' },
        { href: '/hospital-request-acceptance', icon: Hospital, label: 'Hospital Requests', active: pathname === '/hospital-request-acceptance' },
        // { href: '/emergency-call', icon: TriangleAlert, label: 'Call Donors', active: pathname === '/emergency-call' },
        { href: '/dashboard/bloodbank/inventory', icon: Building, label: 'Blood Inventory', active: pathname === '/dashboard/bloodbank/inventory' },
        { href: '/dashboard/bloodbank/find-donors', icon: MapPin, label: 'Find Donors', active: pathname === '/dashboard/bloodbank/find-donors' },
        { href: '/dashboard/bloodbank/donors', icon: Users, label: 'View Donors', active: pathname === '/dashboard/bloodbank/donors' },
        { href: '/dashboard/bloodbank/drives', icon: Droplet, label: 'Donation Drives', active: pathname === '/dashboard/bloodbank/drives' },
        { href: '/dashboard/bloodbank', icon: Activity, label: 'Dashboard', active: pathname === '/dashboard/bloodbank' }
      );
    } else if (isHospital) {
      items.push(
        ...
        { href: '/dashboard/hospital', icon: Activity, label: 'Dashboard', active: pathname === '/dashboard/hospital' }
      );
    }
    ...
    if (userRole === 'admin') {
      items.push(
        { href: '/dashboard/admin', icon: Users, label: 'Admin Dashboard', active: pathname === '/dashboard/admin' }
      );
    }
    ...
    <header className="bg-[var(--card-background)] border-b border-[var(--border-color)] sticky top-0 z-50 w-full shadow-sm transition-colors duration-200">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-18 md:h-20 lg:h-24">
          ...
          {/* Logo */}
          <Link href={isAdmin || isBloodBank ? "/dashboard/admin" : "/"}>
            <button className="flex items-center space-x-2 sm:space-x-2.5 text-xl sm:text-2xl md:text-2xl lg:text-3xl font-bold font-heading text-[#ef4444] hover:text-[#ef4444]/90 transition-colors hover:cursor-pointer">
              ...
              <span>RareBlood Connect</span>
            </button>
          </Link>

          {/* Desktop Navigation */}
          <nav className="hidden lg:flex items-center space-x-1 xl:space-x-6">
            {/* Home - Show for regular users (not admin/bloodbank/hospital) */}
            {!isAdmin && !isBloodBank && !isHospital && (
              <Link href="/">
            ...
            {/* Track Status - Show only for hospital users */}
            {isHospital && (
            ...
            {/* Render Role-Specific Nav Items */}
            {navItems.map((item) => renderNavItem(item, `flex items-center space-x-1.5 px-3 lg:px-4 py-2 lg:py-2.5 rounded-4xl text-sm lg:text-base font-medium transition-colors ${
              item.active 
                ? 'text-[#ef4444] bg-[#ef4444]/5' 
                : 'text-[var(--text-secondary)] hover:text-[#ef4444] hover:bg-[#ef4444]/5'
            }`))}
          ...
          <nav className="hidden md:flex lg:hidden items-center">
            ...
            {navItems.map((item) => renderMediumNavItem(item))}
          </nav>
        ...
        <div className="md:hidden border-t border-[var(--border-color)] bg-[var(--card-background)] transition-colors duration-200">
          <div className="container mx-auto px-4 py-3">
            <nav className="flex flex-col space-y-2">
              ...
              {/* Home - Show for regular users (not admin/bloodbank) */}
              {!isAdmin && !isBloodBank && (
                <Link href="/">
                  <button className={`flex items-center space-x-3 px-4 py-2.5 rounded-4xl text-base font-medium w-full text-left transition-colors ${
                    pathname === '/' 
              ...
              {/* Render Mobile Role-Specific Nav Items */}
