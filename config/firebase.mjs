...
if (!initialized) {
  try {
    ...
    if (fs.existsSync(keyPath)) {
      ...
      console.warn('⚠️ Firebase serviceAccountKey.json not found at project root.');
    }
  } catch (err) {
...
if (!initialized) {
  ...
  // Create a wrapper object instead of trying to override the getter
  adminExport = {
    ...admin,
    messaging: () => ({
      send: async () => {
        console.warn('⚠️ Firebase Admin not initialized - send() is noop');
        return 'noop-id';
      },
